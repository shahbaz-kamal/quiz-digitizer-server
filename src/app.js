...
app.post("/digitalize/process-pdf", upload.single("pdf"), async (req, res) => {
  ...
  for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
    ...
    const diagramInfosOnPage = [];

    parsedPageContent.questions.forEach((q) => {
      if (Array.isArray(q.diagram_info)) {
        diagramInfosOnPage.push(...q.diagram_info);
      }
    });

    // pageNum might be like 1 → format to 01
    ...
    // 🧠 Crop and upload all diagrams
    await processingAndUploadingDiagram({
      diagramInfoList: diagramInfosOnPage,
      pageImagePath,
      pageNum: formattedPageNum,
    });
